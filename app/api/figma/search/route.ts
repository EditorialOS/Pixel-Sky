...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await searchDamAssets(principal.orgId, {
      query: request.nextUrl.searchParams.get('q') ?? '', mode: 'strict', limit: 30,
    });
    return NextResponse.json({
      assets: result.assets.map(({ public_id, filename, preview_url, tags, context, metadata }) => ({ public_id, filename, preview_url, tags, context, metadata })),
      total: result.total,
